...
def file_guide_body() -> str:
    return tr(
        ...
        "Files from the quality check (Check & Refine tab):\n\n"
        "• Quality_Check_1_{name}.txt, Quality_Check_2_… — a readable "
        "report written every time you run “Check profile quality”: the "
        "quality grade, a plain-language explanation, the strips with the "
        "highest error, and the full check output. Numbered so repeated "
        "checks never overwrite each other — a little history of how your "
        "profile improved. Safe to delete any time.\n"
        "• Refine_Strips_{name}.txt — written (and replaced) when a check "
        "flags strips for re-measurement: the list of strips to redo, "
        "which the guided refinement reads back so the Measure tab knows "
        "exactly which strips you need to re-read.\n\n"
