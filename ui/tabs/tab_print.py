...
_TT_BODY_PRINT_MACOS_NATIVE = (
    ...
    "feed, quality, and media type yourself.\n\n"
    "IMPORTANT: in this mode YOU must turn off the printer driver's colour "
    "management. ChromIQ asks the system to disable it, but some drivers "
    "(for example Canon) ignore that and colour-manage the chart anyway, "
    "which makes the measurements useless.\n\n"
    "Before you print:\n"
    ...
    "• In the \"Color Matching\" pane choose ColorSync, not the vendor "
    "name.\n"
    "• Open the driver's own colour section and turn correction off — "
    "Epson \"Color Settings\" → Off (No Color Adjustment), Canon \"Color "
    "Options\" → Color Correction → None, or look for \"No Color "
    "Management\" / \"Application Controlled\".\n"
    "• Pick the right paper / media type and print quality. Do NOT let the "
    "driver apply any ICC profile or rendering intent.\n\n"
    "Not sure, or the driver won't let you turn colour management off? Use "
    "ChromIQ's own printing module instead — untick \"Use the native print "
    "dialog\" in Settings. It prints via lp with colour management forced "
    "off (including the driver's own no-colour option) and is the reliable "
    "choice for profiling.\n\n"
    ...
    def _set_native_mode(self, enabled: bool) -> None:
        ...
        if enabled:
            if _sys.platform == "win32":
                ...
                self._warn_lbl.setText(
                    "⚠  You are printing via the macOS printer dialog. In this mode you must "
                    "disable colour management in the printer driver yourself. ChromIQ asks "
                    "the system to turn it off, but some drivers (for example Canon) ignore "
                    "that and colour-manage the chart anyway — which makes the measurements "
                    "useless.\n\n"
                    "In the print dialog, before printing:\n"
                    "  • Open the \"Color Matching\" pane and choose ColorSync, not the "
                    "vendor's name.\n"
                    "  • Open the driver's own colour section and turn correction off:\n"
                    "      – Epson:  \"Color Settings\" / \"Color Controls\" → Off "
                    "(No Color Adjustment)\n"
                    "      – Canon:  \"Color Options\" → Color Correction → None "
                    "(\"No Color Correction\")\n"
                    "      – Others: look for \"No Color Management\", \"Off\", or "
                    "\"Application Controlled\"\n"
                    "  • Do NOT let the driver apply any ICC profile or rendering intent.\n\n"
                    "Then pick the correct paper / media type and print quality.\n\n"
                    "Not sure, or the driver won't let you turn it off? Use ChromIQ's own "
                    "printing module instead: untick \"Use the native print dialog\" in "
                    "Settings. It sends the chart via lp with colour management forced off "
                    "(including the driver's own no-colour option), which is the reliable "
                    "choice for profiling.\n\n"
                    "Allow pigment inks to dry fully before measuring "
                    "(at least 1 h; 24 h for best accuracy)."
                )
