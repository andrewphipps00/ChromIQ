...
def test_copy_txt_builds_project_layout(tmp_path: Path) -> None:
    ...
    assert new_txt == run1 / "chart.txt"
    assert new_txt.is_file()
    assert base == "chart", "txt2ti3 base must be 'chart' so it writes chart.ti3"
    # No flat <name>.txt at the project root.
    assert not (proj / "Imported.txt").exists()
