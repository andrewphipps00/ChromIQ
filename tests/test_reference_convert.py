...
def test_classify(tmp_path):
    ...
    (tmp_path / "r.cie").write_text("x"); assert classify_reference(tmp_path / "r.cie") is ReferenceKind.DIRECT
    (tmp_path / "r.cxf").write_text("<CxF/>"); assert classify_reference(tmp_path / "r.cxf") is ReferenceKind.CXF
...
class _FakeArgyll:
    ...
    def run(self, cmd, capture_output=True, text=True):
        ...
        if tool == "cxf2ti3":
            Path(cmd[2] + ".ti3").write_text("cie")            # outbase.ti3
        elif tool == "txt2ti3":
            Path(cmd[2] + ".ti3").write_text("spectral ti3")   # tmpbase.ti3
        elif tool == "spec2cie":
            Path(cmd[2]).write_text("cie with xyz")            # out.cie
...
def test_cxf_runs_cxf2ti3(tmp_path):
    ref = tmp_path / "R250715.cxf"; ref.write_text("<CxF/>")
    fk = _FakeArgyll(tmp_path)
    out = convert_reference(ref, fk.bin, tmp_path / "out", runner=fk.run)
...
def test_missing_tool_raises(tmp_path):
    ref = tmp_path / "r.cxf"; ref.write_text("<CxF/>")
